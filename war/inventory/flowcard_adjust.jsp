...
<html>
  ...
  <script type='text/javascript' >
       ...
       var flowcardPath=basePath+'inventory/flowcard!listFlowcardsS';
             ...
             $("#btnNextPage").click( function() {
                 window.self.location=flowcardPath+"?pm.pageSize=<s:property value='#action.pm.pageSize'/>&pm.pageNow=<s:property value='#action.pm.pageNow+1'/>&pm.rowCount=<s:property value='#action.pm.rowCount'/>";
                   ...
                   if (checkedFlowcard.length==0){
                      ...
                      var deleteConfirm=window.confirm("真的要通过审核 这些流向单?");
                      if(deleteConfirm){
                          var url='<%=basePath%>inventory/flowcard!checkFlowcards?searchString='+paramStr+"&pm.pageSize=<s:property value='#action.pm.pageSize'/>&pm.pageNow=<s:property value='#action.pm.pageNow'/>&pm.rowCount=<s:property value='#action.pm.rowCount'/>";
                   ...
                   $("#btnAdjust").click(
                        ...
                        var checkedFlowcard=$(":checkbox[name='fc.id'][checked='true']");
                   if (checkedFlowcard.length==0){
                       alert('请选择要审的流向单，只可以选 一个');
                       return false;
                   } else if(checkedFlowcard.length>1){
                	   alert('一次只可以选 一个');
                       return false;
                   }else {
                          var url='<%=basePath%>inventory/flowcard!proAdjustFlowcard?fc.id='+checkedFlowcard.val()+"&pm.pageSize=<s:property value='#action.pm.pageSize'/>&pm.pageNow=<s:property value='#action.pm.pageNow'/>&pm.rowCount=<s:property value='#action.pm.rowCount'/>";
                            window.self.location=url;
                   }
                           }
                           );
 ...
 */
    ...
    */
    ...
    var urlStr = basePath
            + "inventory/flowcard!searchFlowCardsByClientIdAndFiscalPeriodid?fc.status=S&fc.distrib.id="+ clientId+"&fc.fiscalPeriod.id="+fiscalPeriodId+"&pm.pageSize=<s:property value='#action.pm.pageSize'/>&pm.pageNow=<s:property value='#action.pm.pageNow'/>&pm.rowCount=<s:property value='#action.pm.rowCount'/>";
                ...
                <table width="95%" border="0" cellspacing="0" cellpadding="0"
                    ...
                    <tr>
                        <td width="522" class="p1" height="2" nowrap="nowrap">
                            ...
                            &nbsp;
                            <b>分销商库存管理&gt;&gt;流向单审核</b>  <span style='color: red'> <s:iterator value="#action.actionMessages" var='msg'> <s:property value='#msg' /> </s:iterator><s:iterator value='#action.actionErrors' var ='error' ><s:property  value="#error" /> </s:iterator> </span>
                        </td>
                    </tr>
                </table>
...
<td>    会计核算期 ：</td>
                ...
                <table width="95%" border="1" cellspacing="0" cellpadding="0"
                  ...
                  <s:set var="rows" value="0" />
                    ...
                    <tr>
                        <td width="88" class="rd8">   <s:if test="#fc.fushen!=null"><SPAN  style="color:red">*</span></s:if>
                            <a href="#"
                        ...
                        <div align="right">
                            ...
                            <input class="button1" type="button" id="btnBottomPage"
                                ...
                                <input  class="button1" type="button"
                                    id="btnCheck" value="审核"
                                   >
                                     <input  class="button1" type="button"
                                    id="btnAdjust" value="调整"
                                   >
                ...
                <span  style="position: absolute; left: 30px;"> 标有   <span style="color:red">*</span>  的流向单为未通过复审的流向单</span>
