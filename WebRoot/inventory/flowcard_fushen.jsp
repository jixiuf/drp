    ...
    */
    ...
    //  重置分销商
      ...
      if (checkedFlowcard.length==0){
         ...
         var confirm=window.confirm("真的要通过复审，这些流向单?");
         if(confirm){
             var url='<%=basePath%>inventory/flowcard!fushenFlowcards?searchString='+paramStr+"&pm.pageSize=<s:property value='#action.pm.pageSize'/>&pm.pageNow=<s:property value='#action.pm.pageNow'/>&pm.rowCount=<s:property value='#action.pm.rowCount'/>";
      ...
      $("#rejectFuShenFlowcards").click(
              function(){
           var checkedFlowcard=$(":checkbox[name='fc.id'][checked='true']");
      if (checkedFlowcard.length==0){
          alert('请选择要被驳回的流向单，可以多选');
          return false;
      } else{
          var paramStr="";
          for(var i=0;i<checkedFlowcard.length-1;i++){
          ...
          paramStr+=(checkedFlowcard.get(checkedFlowcard.length-1).value);
         var confirm=window.confirm("真的要驳回这些流向单?");
         if(confirm){
             var url='<%=basePath%>inventory/flowcard!rejectFuShenFlowcards?searchString='+paramStr+"&pm.pageSize=<s:property value='#action.pm.pageSize'/>&pm.pageNow=<s:property value='#action.pm.pageNow'/>&pm.rowCount=<s:property value='#action.pm.rowCount'/>";
               window.self.location=url;
         }
      }
        ...
        success : function(html) {
            $("#searchresult").empty().append($(html));
                ...
                function filterClients()  {
                    ...
                    var url = basePath
                            + "inventory/inventory!listDistribs?distrib.clientno="+clientNo+"&pm.pageSize=<s:property value='#action.pm.pageSize'/>&pm.pageNow=<s:property value='#action.pm.pageNow'/>";
                        ...
                        <div align="right">
                            ...
                            <input class="button1" type="button" id="btnBottomPage"
                                value=" &gt;&gt;|" title="尾页">
                                    ...
                                    id="btnApprove" value="复审"
                                   >
                                        <input  class="button1" type="button"
                                    id="rejectFuShenFlowcards" value="驳回"
                                   >
                            </div>
                        </td>
                    </tr>
