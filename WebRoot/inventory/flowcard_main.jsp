...
<html>
  <head>
    ...
    <script type='text/javascript'  src='js/jquery.js'></script>
       ...
       var flowcardPath=basePath+'inventory/flowcard!listFlowcards';
       ...
       $( function() {
           $("#btnTopPage").click( function() {
             ...
             $("#btnBottomPage").click( function() {
                 var url=flowcardPath+"?pm.pageSize=<s:property value='#action.pm.pageSize'/>&pm.pageNow=<s:property value='#action.pm.pageCount'/>&pm.rowCount=<s:property value='#action.pm.rowCount'/>";
                   window.self.location=url;
               });
                $("#btnAdd").click( function() {
                     var url=basePath+"inventory/flowcard!proSaveFlowcard?pm.pageSize=<s:property value='#action.pm.pageSize'/>&pm.pageNow=<s:property value='#action.pm.pageNow'/>&pm.rowCount=<s:property value='#action.pm.rowCount'/>";
                     var fpId=$("#fiscalPeriodId").val();
                     url=url+"&fiscalPeriodId="+fpId;
                       window.self.location=url;
                   });
                   $("#btnModify").click( function() {
                       var checkboxs= $(":checkbox[name='fc.id'][checked='true']") ;
                       ...
                       var url=basePath+"inventory/flowcard!proModFlowcard?fc.id="+checkboxs.val()+"&pm.pageSize=<s:property value='#action.pm.pageSize'/>&pm.pageNow=<s:property value='#action.pm.pageNow'/>&pm.rowCount=<s:property value='#action.pm.rowCount'/>";
                         window.self.location=url;
                       return true;
                       });
                   ...
                   $("#btnDelete").click(
                           function(){
                        var checkedFlowcard=$(":checkbox[name='fc.id'][checked='true']");
                   if (checkedFlowcard.length==0){
                       alert('请选择要删除的流向单，可以多选');
                       return false;
                   } else{
                       var paramStr="";
                       for(var i=0;i<checkedFlowcard.length-1;i++){
                           paramStr+=(checkedFlowcard.get(i).value+",");
                       }
                       paramStr+=(checkedFlowcard.get(checkedFlowcard.length-1).value);
                      var deleteConfirm=window.confirm("真的要删除这些流向单?");
                      if(deleteConfirm){
                          var url='<%=basePath%>inventory/flowcard!deleteFlowcards?searchString='+paramStr+"&pm.pageSize=<s:property value='#action.pm.pageSize'/>&pm.pageNow=<s:property value='#action.pm.pageNow'/>&pm.rowCount=<s:property value='#action.pm.rowCount'/>";
                            window.self.location=url;
                      }
                   }
                   ...
                   if (checkedFlowcard.length==0){
                       ...
                       paramStr+=(checkedFlowcard.get(checkedFlowcard.length-1).value);
                      var deleteConfirm=window.confirm("真的要送审这些流向单?");
                      if(deleteConfirm){
                          var url='<%=basePath%>inventory/flowcard!sendToCheckInventory?searchString='+paramStr+"&pm.pageSize=<s:property value='#action.pm.pageSize'/>&pm.pageNow=<s:property value='#action.pm.pageNow'/>&pm.rowCount=<s:property value='#action.pm.rowCount'/>";
                            window.self.location=url;
                      }
                   }
                   ...
                   $(":checkbox[name='selectAll']").live("click",
                           function(){
                               ...
                               if(ischecked){
                                   $(":checkbox[name='fc.id']").attr('checked',true);
                               }else{
                                     $(":checkbox[name='fc.id']").attr('checked',false);
                               }
                           }
                        );
    ...
    */
    ...
    //  重置分销商
      ...
      $("#btnResetAll").click(
              function(){
                    var urlStr = basePath
                    + "inventory/flowcard!searchFlowCardsByClientIdAndFiscalPeriodid?fc.distrib.id=&fc.fiscalPeriod.id=&fc.status=L&pm.pageSize=<s:property value='#action.pm.pageSize'/>&pm.pageNow=<s:property value='#action.pm.pageNow'/>&pm.rowCount=<s:property value='#action.pm.rowCount'/>";
                    $.ajax( {
                        url : urlStr,
                        cache : false,
    ...
    var urlStr = basePath
            + "inventory/flowcard!searchFlowCardsByClientIdAndFiscalPeriodid?fc.distrib.id="+ clientId+"&fc.fiscalPeriod.id="+fiscalPeriodId+"&fc.status=L&pm.pageSize=<s:property value='#action.pm.pageSize'/>&pm.pageNow=<s:property value='#action.pm.pageNow'/>&pm.rowCount=<s:property value='#action.pm.rowCount'/>";
                ...
                <table width="95%" border="0" cellspacing="0" cellpadding="0"
                    ...
                    <tr>
                        <td width="522" class="p1" height="2" nowrap="nowrap">
                            ...
                            &nbsp;
                            <b>分销商库存管理&gt;&gt;流向单维护</b>  <span style='color: red'> <s:iterator value="#action.actionMessages" var='msg'> <s:property value='#msg' /> </s:iterator><s:iterator value='#action.actionErrors' var ='error' ><s:property  value="#error" /> </s:iterator> </span>
                        </td>
                    </tr>
                </table>
                          ...
                          <td  >
                            <input name="btnQuery"    type="button" class="button1"
                                id="btnQuery" value="查询"    >
                                  <input name="btnResetAll"    type="button" class="button1"
                                id="btnResetAll" value="列出所有"    >
                         </td>
                    </tr>
